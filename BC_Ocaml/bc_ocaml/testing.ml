...
(* let listy = x :: y :: [];;
let len = List.length listy;;
  print_float (List.nth listy (len-2));; *)
(* let my_hash = Hashtbl.create 44;;
Hashtbl.add my_hash "i" 0.0;;
Hashtbl.replace my_hash "i" ((Hashtbl.find my_hash "i") + 1.0) *)

(* let inc_hash (x : string) : float = 
  Hashtbl.find my_hash "i" *)

  (* while ((Hashtbl.find my_hash "i") < 3.0) do
   Hashtbl.replace my_hash "i" (Hashtbl.find my_hash ("i" + 1.0)) in
   print_float (Hashtbl.find my_hash "i")
  done;; *)
...
print_float (List.nth my_list 0)
;;
print_newline ();;
